...
3. **Author the session** into the scaffolded `session.json`: chapters ordered by review leverage, one review idea per stop, findings/questions marked with `kind` (+ `severity` for findings), mechanical changes into `support`. Respect the length budgets — the validator enforces them.
   ...
   **Priority tiers.** A chapter or stop may carry `"priority": "nice"`; a stop's own tier wins over
   its chapter's, and unset means `must` — nothing becomes skippable by accident. `nice` is for
   internal-facing, easily reversible, low-blast-radius work (dev scripts, admin-page tweaks,
   generated code, mechanical renames). Anything user-facing, hard to reverse, touching
   security/money/data, or wide-reaching stays `must`. Every `nice` owes a one-line why in the
   blurb or prose.
